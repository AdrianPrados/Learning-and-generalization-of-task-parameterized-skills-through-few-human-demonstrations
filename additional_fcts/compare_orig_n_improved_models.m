...
limAxes = [-1.2 2 -1.1 2];
colPegs = [0.25 0.0392 0.2392; 0.9137 0.4980 0.0078];
for new_idx = 1 : nb_repo
    for m=1:models(1).nbFrames
        if m == 1
            pTmp(m).b = s(1).p(m).b;
            pTmp(m).A = s(1).p(m).A;
%             pTmp(m).A = [1 0 0;0 -0.4481 -0.8940;0 0.8940 -0.4481];
        else
            b_new = 0.8 * rand(2,1);
%             b_new = [0.3138;0.5244];
            b_new = [0; b_new];
...
%             rand_angle = 60 + rand * 120;
            rand_angle = deg2rad(rand() * (225 - 45) + 45);
            A_new = rotx(rand_angle);
            A_new(2:3, 2:3) = sqrt(0.1) * A_new(2:3, 2:3);
            A_new(2:3, 2) = -A_new(2:3, 2);
            pTmp(m).A = A_new;  
%             pTmp(m).A = [1 0 0;0 -0.0775 -0.3066;0 -0.3066 0.0775];
%             pTmp(m).b = [0;0.2782;0.1200];
